...
# 🔰 VALIDATION FUNCTION
# -------------------------
def validate_inputs(applicant_income, coapplicant_income, loan_amount, loan_term, property_area):
    errors = []
...
with left_col:
    ...
    # -------------------------
    if st.button("Predict", key="left_predict"):
        if validate_inputs(applicant_income, coapplicant_income, loan_amount, loan_term, property_area):
            prediction, proba = predict(input_encoded)
            if prediction is not None:
                result = "Approved ✅" if prediction[0] == 1 else "Rejected ❌"
                confidence = proba[0][1]
    ...
    if st.button("Make Prediction with Charts", key="right_predict"):
        ...
        if input_encoded is not None:
            ...
            if not validate_inputs(
                ...
                st.warning("❗ Fix validation errors before generating charts.")
                st.stop()
            ...
            if prediction is not None:
                ...
                # -------------------------
                # 📊 Chart 2: Income vs Loan
                # -------------------------
                st.subheader("💰 Income vs Loan Amount")
                total_income = st.session_state.applicant_income + st.session_state.coapplicant_income
                fig2, ax2 = plt.subplots()
                ax2.bar(["Total Income", "Loan Amount"], [total_income, st.session_state.loan_amount], color=["blue", "orange"])
                ax2.set_ylabel("Amount")
                ax2.set_title("Loan vs Income Comparison")
                st.pyplot(fig2)
                ...
                pdf_file = generate_pdf_report(
                    st.session_state.form_data, prediction[0], confidence
                )
